...
interface DashboardData {
    ...
    clicksTrend: Array<{ date: string; count: number }>;
...
export default function DashboardPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const [groups, leads, clicks] = await Promise.all([
                ...
                fetch(`${API_URL}/traffic/clicks/stats`, { headers }).then(r => r.ok ? r.json() : []),
            ]);
            ...
            const groupsNearCapacity = groups.filter((g: any) => g.clickCount >= g.maxClicks * 0.9).length;
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`px-4 py-2 text-sm rounded-lg transition ${timeRange === '7'
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm rounded-lg transition ${timeRange === '30'
                                ? 'bg-[#2C2C2C] text-white'
                                : 'bg-[#F5F3EF] text-[#5C5C5C] hover:bg-[#EBE8E2]'
                            }`}
                    >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white rounded-2xl p-6 border border-[#E8E4DD]">
                    ...
                    <p className="text-xs text-[#9B9B9B] mt-2">Rotator traffic</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-[#E8E4DD]">
                    ...
                    <p className="text-xs text-[#9B9B9B] mt-2">Total signups</p>
                </div>
                <div className="bg-white rounded-2xl p-6 border border-[#E8E4DD]">
                    ...
                    <p className="text-xs text-[#9B9B9B] mt-2">WhatsApp circles</p>
                </div>
                <div className={`rounded-2xl p-6 border ${data?.groupsNearCapacity ? 'bg-[#FEF3E7] border-[#F5D5B5]' : 'bg-white border-[#E8E4DD]'}`}>
                    ...
                    <p className="text-xs text-[#9B9B9B] mt-2">Groups &gt;90% full</p>
                </div>
            </div>
            ...
            {data?.sourceBreakdown && Object.keys(data.sourceBreakdown).length > 0 && (
                <div className="bg-white rounded-2xl p-6 border border-[#E8E4DD]">
                    <h3 className="text-sm font-medium text-[#1A1A1A] mb-4">Traffic Sources</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {Object.entries(data.sourceBreakdown).map(([source, count]) => (
                            <div key={source} className="text-center p-4 bg-[#FAF9F6] rounded-xl">
                                <p className="text-2xl font-semibold text-[#1A1A1A]">{count}</p>
                                <p className="text-xs text-[#9B9B9B] mt-1 uppercase tracking-wide">{source.replace('_', ' ')}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-[#FAF9F6]">
                            <th className="text-left px-6 py-3 text-xs font-medium text-[#9B9B9B] uppercase tracking-wider">Group</th>
                            <th className="text-left px-6 py-3 text-xs font-medium text-[#9B9B9B] uppercase tracking-wider">Capacity</th>
                            <th className="text-left px-6 py-3 text-xs font-medium text-[#9B9B9B] uppercase tracking-wider">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-[#E8E4DD]">
                        {data?.groups?.slice(0, 5).map((group, index) => {
                            ...
                            return (
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-24 h-2 bg-[#E8E4DD] rounded-full">
                                                <div
                                                    className={`h-2 rounded-full ${progress >= 90 ? 'bg-[#D4A574]' : 'bg-[#2C2C2C]'}`}
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${group.isActive
                                                ? 'bg-[#E8F5E8] text-[#4A7C4A]'
                                                : 'bg-[#F5F3EF] text-[#9B9B9B]'
                                            }`}>
